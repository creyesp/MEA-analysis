...
class SyncDigital(Sync):
    def load_analyzed(self, src_folder):
        """Get the digital sinchronization times from MCD file.

        Recovery the exact time when a frame was showed  on the screen
        using LED Lamp software. This create a dital pulse to record
        when a led was turn on.

        Parameters
        ----------
        mcd_channel : int
            channel number where is the digital signal in .mcd file. Use
            showEntities() to check number.
        ...
        """
        digital_data, digital_time, dur = self.get_raw_data(channel)
        # Threshold to delete the basal noise in volts.
        thresh_volts = 3
        ...
        diff_time = np.diff(digital_data)
        start_frame = np.where(diff_time > thresh_volts)[0]
        end_frame = np.where(diff_time < -thresh_volts)[0]
        self.start_end_frame = np.stack((start_frame, end_frame)).T
        self.total_duration = len(digital_data)
